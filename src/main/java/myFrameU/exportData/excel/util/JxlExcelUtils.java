...
public class JxlExcelUtils {
	 ...
	 * @return
	 */
	public static WritableWorkbook exportToExcel(HttpServletResponse response, List<ListOrderedMap> objData, String sheetName,List<String> columns
		...
		try {
			...
			WritableSheet ws = wwb.createSheet(sheetName, wwb.getNumberOfSheets());
...
/*	*//**
	 ...
	 *//*
	public static void exportexcle(HttpServletResponse response,String filename,List<ListOrderedMap> listData,String sheetName,List<String> columns)
	{
		//调用上面的方法、生成Excel文件
		...
		try {
			...
			WritableWorkbook wwb=createWWB(response);
			
			wwb=exportToExcel(response, listData, sheetName, columns,wwb);
			wwb=exportToExcel(response, listData, "dddddddd", columns,wwb);
	...
	public static void writeWWB(HttpServletResponse response,WritableWorkbook wwb){
		try{
			Date now = new Date();
			String name = DateUtils.format(now, DateUtils.formatStr_yyyyMMddHHmm);
			String filename="艺术拍拍-"+name;
			response.setHeader("Content-Disposition", "attachment;filename=" + new String(filename.getBytes("gb2312"), "ISO8859-1") + ".xls");
			//写入Exel工作表
			OutputStream os = response.getOutputStream();
			System.out.println("写入写入写入写入写入写入写入写入写入");
	...
	public static WritableWorkbook createWWB(HttpServletResponse response){
